...
- `uv run --extra train guandan-bc-train data/bc/heuristic.compact.jsonl.gz data/models/bc_ranker.pt --epochs 3 --validation-fraction 0.1 --cache-dir data/bc/heuristic.bc-cache --batch-size 128 --model-architecture dual_tower_v1 --device cuda`: train the behavior-cloning candidate ranker with CUDA from tensor cache.
- `uv run --extra train guandan-ppo-train data/models/ppo_actor_critic.next.pt --init-model data/models/ppo_actor_critic.pt --seed-count 8 --updates 100 --epochs-per-update 3 --max-deals 4 --batch-size 256 --opponent-pool self,heuristic,dummy,previous --reward-shaping-start 0.02 --reward-shaping-end 0.0 --device cuda`: continue self-play PPO fine-tuning with CUDA from the trained PPO actor-critic checkpoint.
